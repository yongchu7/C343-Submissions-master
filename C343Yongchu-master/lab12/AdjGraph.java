...
public void walk2(String method) {
  ...
  for(int index=0; index<totalNode; index+=1) {
    if(!nodeEnum.contains(index)) {
  List<Integer> result = new LinkedList<Integer>();
  List<Integer> history = new Vector<Integer>(nodeEnum);
  BFS(index);
    for(Integer point:nodeEnum) {
  ...
  for(int index=0; index<result2.size(); index+=1) {
    System.out.println("comp" + index + " contains " + result2.get(index).size() + " nodes.");
  }
}
